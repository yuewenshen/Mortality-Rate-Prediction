...
def MAPE(y_true, y_pre):
    ...
    y_pre = (y_pre).detach().numpy().copy().reshape((-1, 1))
    e = (y_true + y_pre) / 2
    re = (np.abs(y_true - y_pre) / (np.abs(y_true) )).mean()*100
    return re
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_len=5000):
        ...
        # pe = pe.t()
        # pe = pe[torch.arange(pe.size(0))!=101].t()
...
class TransAm(nn.Module):
    def __init__(self, feature_size=101*16, num_layers=8, dropout=0.02,hid_size=80,hid_size2=80,hid_size3=80,pp=60):
        ...
        self.decoder = nn.Linear(pp, hid_size)
        self.init_weights()
        self.conv = nn.Conv1d(in_channels=101*16,out_channels=pp,kernel_size=2,stride=1,padding="same")
        self.linear2 = nn.Linear(hid_size,hid_size2)
        self.linear3 = nn.Linear(hid_size2,hid_size3)
        self.linear4 = nn.Linear(hid_size3,feature_size)
    ...
    def forward(self, src):
        ...
        src =src+d
       # src = self.conv((src.permute(0, 2, 1)))
...
data_ = pd.read_excel('F:/csv文件大全/LD_duotou.xlsx', 'Sheet1', parse_dates=["date"])


# if window is 100 and prediction step is 1
# in -> [0..99]
# target -> [1..100]
def create_inout_sequences(input_data, tw):     #  input_data, tw=input_windows   #input-window=100    output_window=5
    inout_seq = []
    L = len(input_data)+1
    # print(L)
    np.zeros((output_window, 3))          #(5,3)
    for i in range(L-tw):
        train_seq = np.append(input_data[i:i + tw, :][:-output_window, :], np.zeros((output_window, 101*16)), axis=0)
...
def get_data():
    ...
    sampels = 50
    train_data = amplitude1[:sampels+5]
    test_data = amplitude1[sampels-13:]
...
def plot(eval_model, data_source, epoch, scaler):
    ...
    #test_result_ = scaler.inverse_transform(test_result_)

    truth_ = truth
    #truth_ = scaler.inverse_transform(truth_)


    #print(truth)
    test_result_1 = test_result_.reshape(16*101*16,).numpy()
    truth_1=truth_.reshape(16*101*16,).numpy()
    test_result_1 = torch.from_numpy(test_result_1)
    truth_1 = torch.from_numpy(truth_1)
    mae =MAE( truth_1, test_result_1)
    mape=MAPE( truth_1,test_result_1)
    rmse=RMSE( truth_1,test_result_1)
    print("mae:",mae)
    print("mape (%)",mape)
    print("rmse:",rmse)
...
epochs = epoch# The number of epochs
best_model = None
